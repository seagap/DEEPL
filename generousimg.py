...
for i in range(1, 5001):
    ...
    a, c = rd.randint(0, x - 50), rd.randint(0, y - 50)
    # print(x, y, a, b, c, d)
    img = np.ones((x, y, 3), dtype=np.uint8) * 255
    # img[a:b, c:d] = np.zeros((b - a, d - c, 3), np.uint8)
    # cv2.waitKey(0)
    img[a:a+50, c:c+50] = np.zeros((50, 50, 3), np.uint8)
    cv2.imwrite('../generousimg/' + str(i) + '.jpg', img)
    # templist = {"x": x, "y": y, "a": a, "b": "b", "c": "c", "d": d,  "cenx": ((c + d) / (2 * y)),"ceny": ((a + b) / (2 * x))}
    templist = {"x": x, "y": y, "a": a, "b": "b", "c": c, "d": "d", "cenx": c+25,
                "ceny":a+25}
